  ...
  const items = [
    {
      ...
      path: "/features/structured-foundation",
    ...
    },
    {
      ...
      path: "/features/native-validation",
    ...
    },
    {
      ...
      path: "/features/guided-execution",
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {items.map((item, index) => (
            <Link 
              to={item.path}
              key={index}
              className="block p-8 rounded-3xl bg-slate-900/50 border border-white/5 hover:bg-slate-900 hover:-translate-y-2 hover:border-indigo-500/30 transition-all duration-300 group animate-fade-up cursor-pointer"
